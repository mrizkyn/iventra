...
const updateProductDetails = (index) => {
    ...
    if (selectedProduct) {
        detail.buy_price_per_unit = selectedProduct.last_buy_price || 0;
    }
};
...
const totalPrice = computed(() => {
    return props.form.details.reduce((total, detail) => {
        const subtotal =
            (detail.quantity || 0) * (detail.buy_price_per_unit || 0);
        detail.subtotal = subtotal; // Update subtotal in detail object
...
<template>
    ...
    >
        <div>
            <InputLabel for="supplier_id" value="Supplier" required />
            <select
                id="supplier_id"
        ...
        <div>
            <InputLabel for="purchase_date" value="Purchase Date" required />
            <TextInput
                id="purchase_date"
                type="date"
                class="mt-1 block w-full"
    ...
    <div class="mt-8 bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        ...
        >
            ...
            <div class="col-span-8 md:col-span-2">
                ...
                <TextInput
                    type="number"
                    v-model="detail.buy_price_per_unit"
                    class="mt-1 w-full"
                />
            </div>
            <div class="col-span-8 md:col-span-2">
                <InputLabel>Sub Total</InputLabel>
                <p class="mt-3 ...">{{ formatCurrency(detail.subtotal) }}</p>
            </div>
            <div class="col-span-4 md:col-span-1 flex items-start">
